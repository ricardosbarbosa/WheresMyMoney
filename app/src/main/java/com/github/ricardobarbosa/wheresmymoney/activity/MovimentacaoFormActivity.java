...
public class MovimentacaoFormActivity extends AppCompatActivity {
    private static final String FORM_FRAGMENT_KEY = "nd_form";

    EnumMovimentacaoTipo tipo;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        String tipoStr = getIntent().getExtras().getString("tipo");
        if (tipoStr!= null) {
            tipo = EnumMovimentacaoTipo.valueOf(tipoStr);
        }

        Bundle arguments = new Bundle();
        arguments.putString("tipo", tipoStr);
        CategoriaDetailFragment fragment = new CategoriaDetailFragment();

        FragmentManager fm = getSupportFragmentManager();
        MovimentacaoFormFragment formFragment;

        Fragment retainedFragment = fm.findFragmentByTag(FORM_FRAGMENT_KEY);
        if (retainedFragment != null && retainedFragment instanceof MovimentacaoFormFragment) {
            formFragment = (MovimentacaoFormFragment) retainedFragment;
            fragment.setArguments(arguments);
        } else {
            formFragment = new MovimentacaoFormFragment();
            fragment.setArguments(arguments);
            fm.beginTransaction()
    ...
    private void setSubmitAction(final MovimentacaoFormFragment formFragment) {
